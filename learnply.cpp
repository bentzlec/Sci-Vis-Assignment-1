...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Scientific Visualization - Connor Bentzley");
...
void keyboard(unsigned char key, int x, int y) {
	...
	case 'y':
	{
		...
		std::cout << "Displaying bi-color map" << std::endl;
	}
	break;
	...
	case 'u':
	{
		...
		std::cout << "Displaying height" << std::endl;
	}
	break;
	...
	case 't':
		...
		for (int i = 0; i < poly->nverts; i++) {
			auto&vertex = poly->vlist[i];
			double s_v = vertex->scalar;
			double gray_s = (s_v - m) / (M - m);
			vertex->R = vertex->G = vertex->B = gray_s;
		}
		glutPostRedisplay();
		std::cout << "Displaying Grayscale" << std::endl;
